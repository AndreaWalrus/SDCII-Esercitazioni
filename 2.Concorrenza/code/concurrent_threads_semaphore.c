...
void* thread_work(void *arg) {
	int i;
	for (i = 0; i < m; i++){
		if(sem_wait(&sem)==-1) fprintf(stderr, "Failed to lock the semaphore, error:%d\n", errno);
		shared_variable += v;
		if(sem_post(&sem)==-1) fprintf(stderr, "Failed to unlock the semaphore, error:%d\n", errno);
	}
	return NULL;
}
...
int main(int argc, char **argv)
{
	...
	if(sem_init(&sem, 0, 1)==-1) fprintf(stderr, "Failed to create semaphore, error:%d\n", errno);
	...
	if(sem_destroy(&sem)==-1) fprintf(stderr, "Failed to destroy semaphore, error:%d\n", errno);
