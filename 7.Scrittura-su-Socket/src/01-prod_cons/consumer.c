...
void openFIFO() {
    ...
    if(fifo==-1) handle_error("FIFO opening error");
...
static void closeFIFO() {
    ...
    int ret;
    ret = close(fifo);
    if(ret!=0) handle_error("FIFO closing error");
    ret = unlink(FIFO_NAME);
    if(ret!=0) handle_error("FIFO unlinking error");
...
int readValue(int * value) {
    ...
    while(bytes_read<sizeof(int)){
        ...
        if(ret==0) handle_error("all pipes are closed");
        if(ret==-1) continue;
        bytes_read+=ret;
    }
