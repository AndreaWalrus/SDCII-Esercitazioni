...
unsigned int process_resource(unsigned int client_id, unsigned int resource_id) {
    ...
    printf("Risorsa %u in queue dal client %u...\n", resource_id, client_id);

    if(sem_wait(&sem_cs[resource_id])!=0) handle_error("sem_wait error");
    ...
    if(sem_post(&sem_cs[resource_id])!=0) handle_error("sem_post error");
...
void* connection_handler(void* arg) {
    ...
    while (1) {
        ...
        //fprintf(stderr, "Received messaged of %d bytes\n", bytes_read);
        ...
        //fprintf(stderr, "Message of %d bytes sent\n", bytes_sent);
    }
...
int main(int argc, char* argv[]) {	  	
    ...
    while (1) { 
        ...
        ret = pthread_create(&thread, NULL, (void*)connection_handler, args);        
        if(ret!=0) handle_error_en(ret, "thread creating error");
        
        fprintf(stderr, "New thread handling the request...\n");

        ret = pthread_detach(thread);
        if(ret!=0) handle_error_en(ret, "thread detaching error");
